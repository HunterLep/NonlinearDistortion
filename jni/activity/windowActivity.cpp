...
const char* windowActivity::getAppName() const{
	return "window.ftu";
}
...
void windowActivity::onCreate() {
    ...
    msys_homePtr = (ZKButton*)findControlByID(ID_WINDOW_sys_home);
	mActivityPtr = this;
	onUI_init();
...
void windowActivity::onIntent(const Intent *intentPtr) {
	Activity::onIntent(intentPtr);
	onUI_intent(intentPtr);
}

bool windowActivity::onTimer(int id) {
	return onUI_Timer(id);
}

void windowActivity::onProgressChanged(ZKSeekBar *pSeekBar, int progress){
...
int windowActivity::getListItemCount(const ZKListView *pListView) const{
...
void windowActivity::obtainListItemData(ZKListView *pListView,ZKListView::ZKListItem *pListItem, int index){
    int tablen = sizeof(SListViewFunctionsCallbackTab) / sizeof(S_ListViewFunctionsCallback);
    for (int i = 0; i < tablen; ++i) {
        if (SListViewFunctionsCallbackTab[i].id == pListView->getID()) {
...
void windowActivity::onItemClick(ZKListView *pListView, int index, int id){
    int tablen = sizeof(SListViewFunctionsCallbackTab) / sizeof(S_ListViewFunctionsCallback);
    for (int i = 0; i < tablen; ++i) {
        if (SListViewFunctionsCallbackTab[i].id == pListView->getID()) {
...
void windowActivity::onSlideItemClick(ZKSlideWindow *pSlideWindow, int index) {
...
bool windowActivity::onTouchEvent(const MotionEvent &ev) {
    return onwindowActivityTouchEvent(ev);
}

void windowActivity::onTextChanged(ZKTextView *pTextView, const std::string &text) {
...
void windowActivity::rigesterActivityTimer() {
...
void windowActivity::onVideoPlayerMessage(ZKVideoView *pVideoView, int msg) {
...
void windowActivity::videoLoopPlayback(ZKVideoView *pVideoView, int msg, size_t callbackTabIndex) {
...
void windowActivity::startVideoLoopPlayback() {
    int tablen = sizeof(SVideoViewCallbackTab) / sizeof(S_VideoViewCallback);
    for (int i = 0; i < tablen; ++i) {
    	if (SVideoViewCallbackTab[i].loop) {
...
void windowActivity::stopVideoLoopPlayback() {
    int tablen = sizeof(SVideoViewCallbackTab) / sizeof(S_VideoViewCallback);
    for (int i = 0; i < tablen; ++i) {
    	if (SVideoViewCallbackTab[i].loop) {
    		ZKVideoView* videoView = (ZKVideoView*)findControlByID(SVideoViewCallbackTab[i].id);
...
bool windowActivity::parseVideoFileList(const char *pFileListPath, std::vector<string>& mediaFileList) {
	mediaFileList.clear();
...
int windowActivity::removeCharFromString(string& nString, char c) {
...
void windowActivity::registerUserTimer(int id, int time) {
	registerTimer(id, time);
}

void windowActivity::unregisterUserTimer(int id) {
	unregisterTimer(id);
}
